...
1. **Load and follow `bicep-to-terraform`.** Invoke the skill and execute its documented Process
   ...
   **Validation gate (required before reporting done):** first complete the skill's semantic and
   apply-safety audits. Verify current provider arguments and output behavior with authoritative
   documentation and `terraform providers schema -json`; use read-only Azure CLI queries when an
   Azure API or principal shape must be confirmed. Do not assume that `terraform validate` catches
   apply-time defects such as service-created child resources, unexported AzAPI response fields,
   parent-operation concurrency, or identity-type requirements.

   From the repo root, run the skill's module layout validator using the implementation and contract
   manifests, then:
   ```bash
   cd infra_tf
   ...
   terraform validate
   ```
   `-backend=false` avoids the not-yet-provisioned state backend. If the audit, layout validator,
   `init`, or `validate` reports an error, fix the generated HCL and re-run the whole gate. Do not
   present the port as complete until every gate succeeds. When you resolve a new class of error,
   add the reusable rule to the skill's `references/bicep-to-terraform-mapping.md` and automate a
   focused check in `scripts/validate-module-layout.sh` when the defect is statically detectable.
...
2. **Honor the skill's hard constraints without exception:**
   ...
   - **Generate apply-safe provider mappings** — prefer native parent-resource arguments; never
     create children that Azure creates automatically; request every AzAPI response field consumed
     by Terraform; serialize mutations when an Azure parent rejects concurrent operations; and
     preserve principal identifier type requirements. Follow the detailed mapping rules and
     validator checks shipped by the skill.
   ...
   - **Pick one flavor per run** — if the entrypoint is a router, inspect its actual allowed
     `deploymentFlavor` values, ask the user which single flavor to port, and follow only that
     branch's module tree while preserving the router's public contract. Inspect the selected
     implementation recursively and the router separately with `--no-recursive`. If only one
     implementation exists or the user already specified the flavor and implementation entrypoint,
...
provider-forced deviation with its reason; the semantic/apply-safety audit and
layout/`fmt`/`validate` results (or that `terraform` was unavailable); confirmation that
`.agent/tmp/` was cleaned; and the next step — point to the Terraform CI/CD skill to generate the
pipeline, noting the state-backend bootstrap is a prerequisite there.
