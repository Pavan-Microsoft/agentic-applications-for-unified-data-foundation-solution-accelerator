---
description: "Create and run an Azure DevOps pipeline for a GitHub-hosted repository, where the code and pipeline YAML stay in GitHub and Azure DevOps only runs the pipeline. Authenticates through the Azure Pipelines GitHub App service connection and creates one pipeline per YAML file with the bundled New-GitHubPipeline.ps1 script (idempotent). Use when users ask to create, add, set up, onboard, register, or wire up an Azure DevOps pipeline for a GitHub repo, connect a GitHub repository to Azure Pipelines, or run ADO CI/CD against code hosted in GitHub. Creates the pipeline pointer only; it does not author YAML, move code, or deploy application code."
name: "create-ado-pipeline-for-github-repo"
---

# Create an Azure DevOps pipeline for a GitHub repository

Create an Azure DevOps pipeline that points to a repository whose code and pipeline YAML stay in GitHub. Azure DevOps only runs the pipeline — no code or YAML is copied into it. Pipelines are created with the `New-GitHubPipeline.ps1` script (`scripts/`) so onboarding is repeatable across repositories.
...
cd .github\skills\create-ado-pipeline-for-github-repo\scripts
```
...
- Script: `.github/skills/create-ado-pipeline-for-github-repo/scripts/New-GitHubPipeline.ps1`
