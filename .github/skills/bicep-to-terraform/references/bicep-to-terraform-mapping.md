...
- AzureRM Search `semantic_search_sku` accepts lowercase `free` or `standard`. Preserve the
  lowercase Bicep value instead of applying `title()`; map Bicep `disabled` to `null`.
...
## Parent-owned and service-created child resources

Some ARM child resources are created automatically with their parent. Modeling them with
`azapi_resource` performs a create operation against an object that already exists, so a fresh
Terraform deployment fails with `Resource already exists`. This is a mapping defect, not an import
requirement:

- Map `Microsoft.Resources/tags/default` into the parent
  `azurerm_resource_group.tags` argument. Do not emit a separate tags resource when Terraform owns
  the resource group.
- Map `Microsoft.Storage/storageAccounts/blobServices/default` properties into the parent
  `azurerm_storage_account` `blob_properties` block. Containers remain normal child resources. If
  an exact blob-service property is unavailable in AzureRM, use `azapi_update_resource` against
  `${azurerm_storage_account.main.id}/blobServices/default`; never create the default service with
  `azapi_resource`.
- For `Microsoft.Web/sites/basicPublishingCredentialsPolicies/{ftp,scm}`, prefer
  `ftp_publish_basic_authentication_enabled` and
  `webdeploy_publish_basic_authentication_enabled` on `azurerm_linux_web_app`. When the site itself
  must use AzAPI, update the automatically created policy children with `azapi_update_resource`.
- Map `Microsoft.Web/sites/config/appsettings` to the parent web app's `app_settings` when AzureRM
  owns the site. For AzAPI-owned sites, update service-created `config/*` children with
  `azapi_update_resource` and an explicit dependency on the parent.

Use import only when adopting infrastructure that existed before the Terraform deployment. Do not
recommend importing children that Azure creates during the same apply.

## AzAPI response outputs

AzAPI exports only the response paths requested by `response_export_values`. Whenever generated HCL
reads `azapi_resource.<name>.output.<path>`, add that exact path to the resource:
...
Prefer targeted paths over `["*"]` to keep state small. Confirm the response shape against the
installed provider schema and the resource API; `terraform validate` cannot prove that Azure will
return an unrequested property.

## Serialized mutations on one parent

Azure Cognitive Services accounts reject concurrent model-deployment mutations with HTTP 409
`Another operation is being performed on the parent resource`. Do not generate a `for_each` module
or resource collection for multiple model deployments under the same account. Emit stable,
explicitly named module/resource blocks and chain them with `depends_on` so each deployment
...
module "embedding_model" {
  source     = "./modules/ai/ai-foundry-model-deployment"
  depends_on = [module.gpt_model]
  # ...
}
```

Apply the same parent-operation audit to other Azure resource types documented as serial. Do not
make `terraform -parallelism=1` the primary fix because the generated dependency graph must express
the platform constraint.

## Fabric capacity administrators

Fabric capacity administration members are identifier-type sensitive:

- Entra users must be supplied by user principal name, such as `user@contoso.com`.
- Service principals and managed identities must be supplied by Entra object ID.

Preserve Bicep's `deployer().userPrincipalName` versus `deployer().objectId` behavior. Do not
unconditionally add `data.azurerm_client_config.current.object_id` to Fabric administrators because
an interactive user's object ID is not a valid substitute for the required UPN. Resolve the
current identifier with a read-only Azure CLI lookup while generating the selected non-secret
...
- **`identity` is a top-level nested block, not an argument or a member of `body`.** Write
  `identity { type = "SystemAssigned" }`, never `identity = { type = "SystemAssigned" }`. The
  latter fails validate with *"An argument named identity is not expected here"*. The same applies
  to `timeouts`.
- **`azapi_update_resource` cannot set identity:** configure identity on the owning resource or use
  `azapi_resource`.
