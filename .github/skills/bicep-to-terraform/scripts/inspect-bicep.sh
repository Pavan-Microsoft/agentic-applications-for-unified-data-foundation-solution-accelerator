  ...
  jq '
    ...
    | map(
        ...
        | . + (
          ...
          elif ($azapi_update_required | index($type)) then
            {hint: "azapi_update_required",
             reason: "service-created child; update it or use the parent provider resource"}
