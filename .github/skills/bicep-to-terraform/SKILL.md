...
2. **Inspect recursively.** Run `bash <absolute-skill-path>/scripts/inspect-bicep.sh
   ...
   - **`providerHints[]`** is one verdict per resource type: `azapi_required` (no stable azurerm
     resource — use `azapi_resource`), `azapi_update_required` (the Azure service creates the child,
     so update it rather than creating it), `azapi_candidate_preview` (preview api-version; confirm
     provider coverage), or `azurerm_expected`. The manifest root repeats every
     non-`azurerm_expected` verdict under `azapiRequiredTypes` so the provider-specific work is
     visible up front. Treat these as starting points and confirm against the installed provider,
...
8. **Validate — mandatory semantic and static gates; iterate until clean.** Trace every `count` and
   ...
   translations and provider deprecations. Perform an apply-safety audit that rejects:
   - `azapi_resource` creation of parent-owned or service-created tags, blob-service defaults, Web
     publishing policies, and Web `config/*` children;
   - any `azapi_resource.<name>.output.<path>` read without a matching targeted
     `response_export_values` entry;
   - concurrent `for_each` model deployments under one Cognitive Services account instead of an
     explicit dependency chain; and
   - Fabric user administrators represented by object ID instead of user principal name.

   Use the web tool for current authoritative Azure/provider documentation. After provider
   initialization, use `terraform providers schema -json` through the execute tool to verify exact
   resource arguments and output controls; use read-only `az` queries when an API or principal
   shape must be confirmed. Additional tools are not a substitute for this audit.

   Then run this sequence and **do not consider the port complete until the semantic audit,
   structure validation, and `terraform validate` succeed**:
   ```bash
